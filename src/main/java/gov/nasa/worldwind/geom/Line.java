...
public final class Line// Instances are immutable
{
	...
	/**
	 * Performs a comparison to test whether this Object is internally
	 * identical to the other Object <code>o</code>. This method takes into
	 * account both direction and origin, so two lines which may be
	 * equivalent may not be considered equal.
	 *
...
//    }
	 ...
	 *
	 * @param p the <code>Vec4</code> whose distance from this
	 * <code>Line</code> will be calculated
	 *
	 ...
	 * @param p The point outside the segment whose closest point on the
	 * segment is desired.
	 *
	 * @return The closest point on (p0, p1) to p. Note that this will be p0
	 * or p1 themselves whenever the closest point on the <em>line</em>
	 * defined by p0 and p1 is outside the segment (i.e., the results are
	 * bounded by the segment endpoints).
	 */
	 ...
	 *
	 * @return The two points at which the segment intersects the frustum,
	 * or null if the segment does not intersect and the frustum does not
	 * fully contain it. If the segment is coincident with a plane of the
	 * frustum, the returned segment is the portion of the original segment
	 * on that plane, clipped to the other frustum planes.
	 */
