...
class UTMCoordConverter {
	 ...
	 * UTM zone override parameter as inputs, and sets the corresponding
	 * state variables. If any errors occur, the error code(s) are returned
	 ...
	 * longitude) coordinates to UTM projection (zone, hemisphere, easting
	 * and northing) coordinates according to the current ellipsoid and UTM
	 * zone override parameters. If any errors occur, the error code(s) are
	 * returned by the function, otherwise UTM_NO_ERROR is returned.
	 *
	 ...
	 * parameters. If any errors occur, the error code(s) are returned by
