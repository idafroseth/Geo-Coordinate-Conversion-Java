...
public class Vec4 {
	 ...
	 * double array. The specified offset must be 0 or greater, the
	 * specified length must be 1 or greater, and the array must have
	 * capacity equal to or greater than <code>offset + length</code>.
	 * Coordinates are assigned as follows:
	 * <p>
	 ...
	 *
	 * @return a new Vec4 with coordinate values read from the specified
	 * double array.
	 *
	 * @throws IllegalArgumentException if the array is null, if offset is
	 * negative, if length is less than 1, or if the array's capacity is
	 * less than <code>offset + length</code>.
	 */
	 ...
	 * length must be 1 or greater, and the array must have capacity equal
	 * to or greater than <code>offset + length</code>. Coordinates are
	 * assigned as follows:
	 * <p>
	 * <code>x = array[offset]</code><br/> <code>y
	 * = array[offset + 1]</code> if <code>length > 1</code>, otherwise <code>y=0</code><br/><code>z = array[offset +
	...
	/**
	 * Constructs a new Vec4 with <code>x</code> and <code>y</code> values
	 * from the specified double array. The specified offset must be 0 or
	 ...
	 * offset must be 0 or greater, and the array must have capacity equal
	 * to or greater than <code>offset + 3</code>. Coordinates are assigned
	 * as follows:
	 * <p>
	 * <code>x = array[offset]</code><br/><code>y =
	 ...
	 * @param offset the array starting index.
	 *
	 * @return a new Vec4 with <code>x</code>, <code>y</code> and
	 * <code>z</code> coordinate values from the specified double array.
	 *
	 * @throws IllegalArgumentException if the array is null, if offset is
	 * negative, or if the array's capacity is less than
	 * <code>offset + 3</code>.
	...
	/**
	 * Constructs a new Vec4 with <code>x</code>, <code>y</code>,
	 * <code>z</code> and <code>w</code> values from the specified double
	 * array. The specified offset must be 0 or greater, and the array must
	 * have capacity equal to or greater than <code>offset + 4</code>.
	 * Coordinates are assigned as follows:
	 * <p>
	 * <code>x =
	 ...
	 * @param offset the array starting index.
	 *
	 * @return a new Vec4 with <code>x</code>, <code>y</code>,
	 * <code>z</code> and <code>w</code> coordinate values from the
	 ...
	 * The specified offset must be 0 or greater, the specified length must
	 * be 1 or greater, and the array must have capacity equal to or greater
	 * than <code>offset
	 ...
	 * 1]</code> is not written to<br/> <code>array[offset + 2] = z</code>
	 * if <code>length > 2</code>, otherwise <code>array[offset + 2]</code>
	 * is not written to<br/><code>array[offset + 3] = w</code> if <code>length >
	 * 3</code>, otherwise <code>array[offset + 3]</code> is not written
	 * to</p>
	 *
	 ...
	 * specified double array. The specified offset must be 0 or greater,
	 * and the array must have have capacity equal to or greater than
	 * <code>offset + 2</code>. Coordinates are written to the array as
	 * follows:
	 * <p>
	 ...
	 * values to the specified double array. The specified offset must be 0
	 * or greater, and the array must have have capacity equal to or greater
	 * than <code>offset + 3</code>. Coordinates are written to the array as
	 * follows:
	 * <p>
	 * <code>array[offset] =
	 ...
	 * <code>w</code> values to the specified double array. The specified
	 * offset must be 0 or greater, and the array must have have capacity
	 * equal to or greater than <code>offset + 4</code>. Coordinates are
	 * written to the array as follows:
	 * <p>
	 * <code>array[offset] =
	 ...
	 * point suitable for use where four-dimensional homogeneous coordinates
	 * are required. The returned vector has <code>x y z</code> coordinates
	 * are equal to this vector's <code>x y z</code> coordinates, and has
	 * <code>w</code> coordinate equal to 1.0.
	 * <p/>
	 * A three-dimensional point in homogeneous coordinates is necessary
	 * when transforming that point by a 4x4 transformation matrix, or when
	 * calculating the dot product of the point and the equation of a plane.
	 * The returned vector is affected by the translation component of a 4x4
	...
	/**
	 * Returns a representation of this vector as an <code>x y z</code>
	 * direction suitable for use where four-dimensional homogeneous
	 * coordinates are required. The returned vector has <code>x y z</code>
	 * coordinates are equal to this vector's <code>x y z</code>
	 * coordinates, and has <code>w</code> coordinate equal to 0.0.
	 * <p/>
	 ...
	 * transformation matrix, and therefore remains invariant under
	 * translation.
	 *
...
//    }
	 ...
	 * other.
	 *
	 * @return an array of two unit vectors mutually orthogonal to this
	 * vector.
	 */
	 ...
	 * specified points Iterable. This returns null if the Iterable contains
	 * no points, or if all of the points are null.
	 *
	 * @param points the Iterable of points which define the returned
	 * arithmetic mean.
	 *
	 * @return the arithmetic mean point of the specified points Iterable,
	 * or null if the Iterable is empty or contains only null points.
	 *
	 ...
	 * resulting array is always of length 2, with the first element
	 * containing the minimum extremum, and the second containing the
	 * maximum. The minimum extremum is composed by taking the smallest x, y
	 * and z values from all the <code>Vec4</code>s in the array. These
	 * values are not necessarily taken from the same <code>Vec4</code>. The
	 * maximum extrema is composed in the same fashion.
	 *
	 * @param points any array of <code>Vec4</code>s
	 *
	 * @return a array with length of 2, comprising the most extreme values
	 * in the given array
	 *
