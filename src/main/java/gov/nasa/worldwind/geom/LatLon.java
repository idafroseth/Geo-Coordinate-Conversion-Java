 ...
 *
 * @author Tom Gaskins
 * @version $Id: LatLon.java 3312 2015-07-09 00:18:11Z tgaskins $
 */
public class LatLon {
	...
	/**
	 * Factory method for obtaining a new <code>LatLon</code> from two
	...
	/**
	 * Constructs a new  <code>LatLon</code> from two angles. Neither angle
	 * may be null.
	 *
	 ...
	 * <code>value2</code>, according to the specified path type. If the
	 * path type is {@link AVKey#GREAT_CIRCLE} this returns an interpolated
	 * value on the great arc that spans the two locations (see
	 * {@link #interpolateGreatCircle(double, LatLon, LatLon)}). If the path
	 * type is {@link AVKey#RHUMB_LINE} or {@link AVKey#LOXODROME} this
	 * returns an interpolated value on the rhumb line that spans the two
	 * locations (see {@link #interpolateRhumb(double, LatLon, LatLon)}.
	 * Otherwise, this returns the linear interpolation of the two locations
	 * (see {@link #interpolate(double, LatLon, LatLon)}.
	 *
	 ...
	 *
	 * @throws IllegalArgumentException if the path type or either location
	 * is null.
	 */
	 ...
	 * clamped to the range [0, 1]. If <code>a</code> is 0 or less, this
	 * returns <code>value1</code>. If <code>amount</code> is 1 or more,
	 * this returns <code>value2</code>. Otherwise, this returns the
	 * location on the great-arc between <code>value1</code> and
	 * <code>value2</code> corresponding to the specified interpolation
	 * factor. This method uses a spherical model, not elliptical.
	 *
	 * @param amount the interpolation factor
	 * @param value1 the first location.
	 * @param value2 the second location.
	 ...
	 * returns <code>value1</code>. If <code>amount</code> is 1 or more,
	 * this returns <code>value2</code>. Otherwise, this returns the
	 * location on the rhumb line between <code>value1</code> and
	 * <code>value2</code> corresponding to the specified interpolation
	 * factor. This method uses a spherical model, not elliptical.
	 *
	 * @param amount the interpolation factor
	 ...
	 * return value gives the distance as the angle between the two
	 * positions on the pi radius circle. In radians, this angle is also the
	 * arc length of the segment between the two positions on that circle.
	 * To compute a distance in meters from this value, multiply it by the
	 * radius of the globe. This method uses a spherical model, not
	 * elliptical.
	 *
	 * @param p1 LatLon of the first location
	 * @param p2 LatLon of the second location
	 *
	 * @return the angular distance between the two locations. In radians,
	...
	/**
	 * Computes the azimuth angle (clockwise from North) that points from
	 * the first location to the second location. This angle can be used as
	 * the starting azimuth for a great circle arc that begins at the first
	 ...
	 * @param p2 LatLon of the second location
	 *
	 * @return Angle that points from the first location to the second
	 * location.
	 */
	 ...
	 * location, azimuth, and arc distance. This method uses a spherical
	 ...
	 *
	 * @param p LatLon of the starting location
	 * @param greatCircleAzimuthRadians great circle azimuth angle
	...
	/**
	 * Returns two locations with the most extreme latitudes on the great
	 * circle with the given starting location and azimuth. This method uses
	 ...
	 *
	 * @return two locations where the great circle has its extreme
	 * latitudes.
	 *
	...
	/**
	 * Returns two locations with the most extreme latitudes on the great
	 * circle arc defined by, and limited to, the two locations. This method
	 ...
	 *
	 * @return two locations with the most extreme latitudes on the great
	 * circle arc.
	 *
	...
	/**
	 * Returns two locations with the most extreme latitudes on the sequence
	 * of great circle arcs defined by each pair of locations in the
	 * specified iterable. This method uses a spherical model, not
	 * elliptical.
	 *
	...
	/**
	 * Computes the length of the rhumb line between two locations. The
	 * return value gives the distance as the angular distance between the
	 * two positions on the pi radius circle. In radians, this angle is also
	 * the arc length of the segment between the two positions on that
	 * circle. To compute a distance in meters from this value, multiply it
	 * by the radius of the globe. This method uses a spherical model, not
	 * elliptical.
	 *
	 * @param p1 LatLon of the first location
	 * @param p2 LatLon of the second location
	 *
	 * @return the arc length of the rhumb line between the two locations.
	 * In radians, this value is the arc length on the radius pi circle.
	 */
	...
	/**
	 * Computes the azimuth angle (clockwise from North) of a rhumb line (a
	 * line of constant heading) between two locations. This method uses a
	 * spherical model, not elliptical.
	 *
	 * @param p1 LatLon of the first location
	 * @param p2 LatLon of the second location
	...
	/**
	 * Computes the location on a rhumb line with the given starting
	 * location, rhumb azimuth, and arc distance along the line. This method
	 * uses a spherical model, not elliptical.
	 *
	 * @param p LatLon of the starting location
	 ...
	 *
	 * @param p LatLon of the starting location
	 * @param rhumbAzimuthRadians rhumb azimuth angle (clockwise from
	 * North), in radians
	 * @param pathLengthRadians arc distance to travel, in radians
	 *
	 * @return LatLon location on the rhumb line.
	...
	/**
	 * Computes the length of the linear path between two locations. The
	 * return value gives the distance as the angular distance between the
	 * two positions on the pi radius circle. In radians, this angle is also
	 * the arc length of the segment between the two positions on that
	 * circle. To compute a distance in meters from this value, multiply it
	 * by the radius of the globe.
	 *
	 * @param p1 LatLon of the first location
	 * @param p2 LatLon of the second location
	 *
	 * @return the arc length of the line between the two locations. In
	...
	/**
	 * Computes the azimuth angle (clockwise from North) of a linear path
	 * two locations.
	 *
	 * @param p1 LatLon of the first location
	 * @param p2 LatLon of the second location
	...
	public static Angle linearAzimuth(LatLon p1, LatLon p2) {
		if (p1 == null || p2 == null) {
			...
			throw new IllegalArgumentException("LatLonIsNull");
		}
	 ...
	 * azimuth, and arc distance along the line. A linear path is determined
	 * by treating latitude and longitude as a rectangular grid. This type
	 * of path is a straight line in the equidistant cylindrical map
	 * projection (also called equirectangular).
	 *
	 * @param p LatLon of the starting location
	...
	public static LatLon linearEndPosition(LatLon p, Angle linearAzimuth, Angle pathLength) {
		...
		if (linearAzimuth == null || pathLength == null) {
			throw new IllegalArgumentException("AngleIsNull");
		}
	...
	/**
	 * Computes the average distance between a specified center point and a
	 * list of locations.
	 *
	 ...
	 *
	 * @throws java.lang.IllegalArgumentException if any of the specified
	 * globe, center or locations are null.
	 *
	 * public static Angle getAverageDistance(Globe globe, LatLon center,
	 * Iterable<? extends LatLon> locations) { if ((globe == null)) { String
	 * msg = Logging.getMessage("nullValue.GlobeIsNull");
	 * Logging.logger().severe(msg); throw new
	 * IllegalArgumentException(msg); }
	 *
	 * if ((center == null)) { String msg =
	 * Logging.getMessage("nullValue.LatLonIsNull");
	 * Logging.logger().severe(msg); throw new
	 * IllegalArgumentException(msg); }
	 *
	 * if ((locations == null)) { String msg =
	 * Logging.getMessage("nullValue.LocationsListIsNull");
	 * Logging.logger().severe(msg); throw new
	 * IllegalArgumentException(msg); }
	 *
	 * int count = 0; for (LatLon ignored : locations) { ++count; }
	 *
	 * Vec4 centerPoint = globe.computeEllipsoidalPointFromLocation(center);
	 *
	 * double totalDistance = 0; for (LatLon location : locations) { double
	 * distance =
	 * globe.computeEllipsoidalPointFromLocation(location).subtract3(centerPoint).getLength3();
	 * totalDistance += distance / count; }
	 *
	 * return (count == 0) ? Angle.ZERO : Angle.fromRadians(totalDistance /
	 ...
	 *
	 * @throws java.lang.IllegalArgumentException if the specified locations
	 * is null.
	 */
	public static LatLon getCenter(Iterable<? extends LatLon> locations) {
		if ((locations == null)) {

			throw new IllegalArgumentException("LocationsListIsNull");
		}
	 ...
	 *
	 * int count = 0; for (LatLon location : locations) { center =
	 * center.add3(globe.computeEllipsoidalPointFromLocation(location));
	 * ++count; }
	 *
	 * return
	 * globe.computePositionFromEllipsoidalPoint(center.divide3(count)); }
	 */
	 ...
	 * North or South pole. The sequence is treated as a closed loop. (If
	 * the first and last positions are not equal the loop will be closed
	 ...
	 * boundary. If the sequence does not cross the dateline this returns a
	 * list containing a copy of the original list.
	 *
	...
	/**
	 * Transform the negative longitudes of a dateline-spanning location
	 * list to positive values that maintain the relationship with the other
	 * locations in the list. Negative longitudes are transformed to values
	 * greater than 180 degrees, as though longitude spanned [0, 360] rather
	 * than [-180, 180]. This enables arithmetic operations to be performed
	 * on the locations without having to take into account the longitude
	 * jump at the dateline.
	 *
	 * @param locations the locations to transform. This list is not
	 * modified.
	 *
	...
	/**
	 * Determine where a line between two locations crosses a given
	 * meridian. The intersection test is performed by treating geographic
	 * coordinates as linear and computing the intersection of the linear
	 * segment with the vertical line indicated by the meridian. This
	 * computation correctly handles intersections with either side of the
	 * antimeridian.
	 *
	 ...
	 *
	 * @throws java.lang.IllegalArgumentException if either location is
	 * null, or if the meridian is null.
	 */
	public static LatLon intersectionWithMeridian(LatLon p1, LatLon p2, Angle meridian) {
		if (p1 == null || p2 == null) {

			throw new IllegalArgumentException("LocationIsNull");
		}

		if (meridian == null) {
			throw new IllegalArgumentException("MeridianIsNull");
		}
	...
	/**
	 * Parses a string containing latitude and longitude coordinates in
	 * either Degrees-minutes-seconds or decimal degrees. The latitude must
	 * precede the longitude and the angles must be separated by a comma.
	 *
	 * @param latLonString a string containing the comma separated latitude
	 * and longitude in either DMS or decimal degrees.
	 *
	 * @return a <code>LatLon</code> instance with the parsed angles.
	 *
	 * @throws IllegalArgumentException if <code>latLonString</code> is
	 * null.
	 ...
	 * <p/>
	 * Algorithm from U.S. National Geodetic Survey, FORTRAN program
	 * "inverse," subroutine "INVER1," by L. PFEIFER and JOHN G. GERGEN. See
	 * http://www.ngs.noaa.gov/TOOLS/Inv_Fwd/Inv_Fwd.html
	 * <p/>
	 * Original documentation: SOLUTION OF THE GEODETIC INVERSE PROBLEM
	 * AFTER T.VINCENTY MODIFIED RAINSFORD'S METHOD WITH HELMERT'S
	 * ELLIPTICAL TERMS EFFECTIVE IN ANY AZIMUTH AND AT ANY DISTANCE SHORT
	 * OF ANTIPODAL STANDPOINT/FOREPOINT MUST NOT BE THE GEOGRAPHIC POLE
	 * <p/>
	 * Requires close to 1.4 E-5 seconds wall clock time per call on a 550
	 * MHz Pentium with Linux 7.2.
	 * <p>
	 * The algorithm used is iterative and will iterate only 10 times if it
	 * does not converge.
	 * </p>
	 *
	 * @param p1 first position
	...
	/**
	 * Computes a new set of locations translated from a specified location
	 * to a new location.
	 *
	 ...
	 *
	 * @return the translated locations, or null if the locations could not
	 * be translated.
	 *
