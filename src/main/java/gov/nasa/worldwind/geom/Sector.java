...
public class Sector implements Comparable<Sector>, Iterable<LatLon> {
	 ...
	 * latitude and +/- 180 degrees longitude, but this method does not
	 * verify that.
	 *
	 ...
	 * specified array. The array is assumed to hold four elements
	 * containing the Sector's angles, and must be ordered as follows:
	 * minimum latitude, maximum latitude, minimum longitude, and maximum
	 * longitude. Additionally, the angles are assumed to be normalized to
	 * +/- 90 degrees latitude and +/- 180 degrees longitude, but this
	 ...
	 *
	 * @throws IllegalArgumentException if <code>array</code> is null or if
	 * its length is less than 4.
	 */
	 ...
	 *
	 * @param rectangle the sector's rectangle in degrees lat-lon
	 * coordinates.
	 *
	 * @return the new <code>Sector</code>
	 */
	...
	/**
	 * Creates a new <code>Sector</code> and initializes it to the specified
	 * angles. The angles are assumed to be normalized to +/- \u03c0/2
	 * radians latitude and +/- \u03c0 radians longitude, but this method
	 ...
	 * The UTM rectangle is located in specified UTM zone and hemisphere.
	 *
	...
	/**
	 * Returns the angular difference between the sector's minimum and
	 * maximum latitudes: max - min
	 *
	 * @return The angular difference between the sector's minimum and
	 * maximum latitudes.
	 */
	 ...
	 * (minimum latitude + maximum latitude) / 2, (minimum longitude +
	 * maximum longitude) / 2.
	 *
	...
	/**
	 * Determines whether a latitude/longitude position is within the
	 * sector. The sector's angles are assumed to be normalized to +/- 90
	 * degrees latitude and +/- 180 degrees longitude. The result of the
	 * operation is undefined if they are not.
	 *
	 * @param latLon the position to test, with angles normalized to +/-
	 * &#960 latitude and +/- 2&#960 longitude.
	 *
	...
	/**
	 * Determines whether a latitude/longitude postion expressed in radians
	 * is within the sector. The sector's angles are assumed to be
	 * normalized to +/- 90 degrees latitude and +/- 180 degrees longitude.
	 * The result of the operation is undefined if they are not.
	 *
	 * @param radiansLatitude the latitude in radians of the position to
	 * test, normalized +/- &#960.
	 * @param radiansLongitude the longitude in radians of the position to
	 * test, normalized +/- 2&#960.
	 *
	 * @return <code>true</code> if the position is within the sector,
	 * <code>false</code> otherwise.
	 */
	 ...
	 * latitude and +/- 180 degrees longitude. The result of the operation
	 * is undefined if they are not.
	 *
	 * @param that the sector to test for containment.
	 *
	 * @return <code>true</code> if this sector fully contains the input
	 ...
	 * latitude and longitude. The sector's angles are assumed to be
	 * normalized to +/- 90 degrees latitude and +/- 180 degrees longitude.
	 * The result of the operation is undefined if they are not.
	 *
	 ...
	 * @param that the sector to test for intersection.
	 *
	 * @return <code>true</code> if the sectors' interiors intersect,
	...
	/**
	 * Determines whether this sector intersects the specified geographic
	 * line segment. The line segment is specified by a begin location and
	 * an end location. The locations are are assumed to be connected by a
	 * linear path in geographic space. This returns true if any location
	 * along that linear path intersects this sector, including the begin
	 * and end locations.
	 *
	 ...
	 *
	 * @throws IllegalArgumentException if either the begin location or the
	...
	/**
	 * Determines whether this sector intersects any one of the sectors in
	 * the specified iterable. This returns true if at least one of the
	 * sectors is non-null and intersects this sector.
	 *
	 * @param sectors the sectors to test for intersection.
	 *
	 * @return true if at least one of the sectors is non-null and
	 * intersects this sector, otherwise false.
	 *
	...
	/**
	 * Returns a new sector whose angles are the extremes of the this sector
	 ...
	 * assumed to be normalized to +/- 90 degrees latitude and +/- 180
	 * degrees longitude. The result of the operation is undefined if they
	 * are not.
	 *
	 ...
	 * This returns a non-null sector if the iterable contains at least one
	 * non-null entry and all non-null entries intersect. The returned
	 * sector represents the geographic region in which all sectors
	 * intersect. This returns null if at least one of the sectors does not
	...
	/**
	 * Returns a four element array containing the Sector's angles in
	 * degrees. The returned array is ordered as follows: minimum latitude,
	...
	/**
	 * Retrieve the size of this object in bytes. This implementation
	 * returns an exact value of the object's size.
	 *
	 ...
	 * respectively.
	 *
	 * @param that the <code>Sector</code> to compareTo with
	 * <code>this</code>.
	 *
	 * @return -1 if this sector compares less than that specified, 0 if
	 * they're equal, and 1 if it compares greater.
	 *
	...
	/**
	 * Creates an iterator over the four corners of the sector, starting
	 * with the southwest position and continuing counter-clockwise.
	 *
	...
	/**
	 * Returns the coordinates of the sector as an array of values in
	 * degrees, in the order minLat, maxLat, minLon, maxLon.
	 *
	...
	/**
	 * Returns the coordinates of the sector as an array of values in
	 * radians, in the order minLat, maxLat, minLon, maxLon.
	 *
	 * @return the array of sector coordinates.
	 */
	 ...
	 *
	 * @return an array of the four corner locations, in the order SW, SE,
	 * NE, NW
	 */
	...
	/**
	 * Tests the equality of the sectors' angles. Sectors are equal if all
	 * of their corresponding angles are equal.
	 *
	 * @param o the sector to compareTo with <code>this</code>.
	 *
	 * @return <code>true</code> if the four corresponding angles of each
	 * sector are equal, <code>false</code> otherwise.
	 */
	@Override
