...
public interface AVKey // TODO: Eliminate unused constants, if any
{
	...
	/**
	 * Indicates the primitive data type of a dataset or a buffer of data.
	 * When used as a key, the corresponding value may be one of the
	 * following: <code>INT8</code>, <code>INT16</code>, <code>INT32</code>,
	...
	/**
	 * Indicates the World Wind scene's worst-case depth resolution, in
	 * meters. This is typically interpreted by the View as the desired
	 * resolution at the scene's maximum drawing distance. In this case, the
	 * resolution closer to the viewer's eye point is significantly better
	 * then the worst-case resolution. Decreasing this value enables the
	 * viewer to get closer to 3D shapes positioned above the terrain at the
	 * coast of potential rendering artifacts between shapes that are places
	 * closely together or close to the terrain.
	 */
	 ...
	 * resource before attempts are suspended. Attempts are restarted after
	 * the interval specified by {@link #MIN_ABSENT_TILE_CHECK_INTERVAL}.
	 *
	...
	/**
	 * Indicates the location that MIL-STD-2525 tactical symbols and
	 * tactical point graphics retrieve their icons from. When used as a
	 * key, the corresponding value must be a string indicating a URL to a
	 * remote server, a URL to a ZIP/JAR file, or a path to folder on the
	 * local file system.
	 */
	...
	/**
	 * Indicates a list of one or more object's positions in a series. When
	 * used as a key, the corresponding value must be a
	 * {@link java.util.List} of {@link Integer} objects indicating the
	 * ordinals.
	 */
	...
	/**
	 * Indicates whether a raster's pixel values represent imagery or
	 * elevation data. When used as a key, the corresponding value may be
	 * one of the following: <code>IMAGERY</code> or <code>ELEVATION</code>.
	 */
	...
	/**
	 * On window resize, scales the item to occupy a constant relative size
	 * of the viewport.
	 */
	final String RESIZE_STRETCH = "gov.nasa.worldwind.CompassLayer.ResizeStretch";
	/**
	 * On window resize, scales the item to occupy a constant relative size
	 * of the viewport, but not larger than the item's inherent size scaled
	 * by the layer's item scale factor.
	 */
